...
class _CourseDetailScreenState extends State<CourseDetailScreen>
  ...
  String? _extractYoutubeId(Map<String, dynamic> item) {
    // Prefer explicit youtube_id, else parse from the url field.
    final direct = item['youtube_id']?.toString().trim();
    ...
    // youtube.com/live/ID, youtube.com/embed/ID (with optional query params)
    final patterns = [
  ...
  void _openContent(Map<String, dynamic> item) {
    ...
    if (type == 'video') {
      final ytId = _extractYoutubeId(item);
      if (ytId != null) {
        Navigator.push(context, MaterialPageRoute(
          builder: (_) => VideoPlayerScreen(youtubeId: ytId, title: item['title'] ?? 'Video'),
        ));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
