...
class _MockTestPlayerScreenState extends State<MockTestPlayerScreen> {
      ...
      body: Column(
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Text('Question ${_currentIndex + 1}/${_questions.length}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: isDark ? Colors.white : Colors.black87)),
                const Spacer(),
                Text('+$posMarks', style: const TextStyle(color: Colors.green, fontWeight: FontWeight.bold, fontSize: 14)),
                const SizedBox(width: 10),
                Text('-$negMark', style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold, fontSize: 14)),
              ],
            ),
          ),
          // Question + options
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                const SizedBox(height: 20),
                // Options
                ...options.entries.map((e) => _optionTile(e.key, e.value, selected, isDark)),
                const SizedBox(height: 20),
              ],
            ),
          ),
  ...
  Widget _optionTile(String key, String value, String? selected, bool isDark) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected ? AppColors.primary.withOpacity(0.15) : (isDark ? const Color(0xFF1C1C1E) : Colors.white),
          borderRadius: BorderRadius.circular(14),
          border: Border.all(color: isSelected ? AppColors.primary : (isDark ? Colors.white12 : Colors.black12), width: 1.5),
        ),
        child: Row(
          children: [
            Container(
              ...
              child: Center(
                child: isSelected
                    ...
                    : Text(key, style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.primary)),
              ),
            ),
            const SizedBox(width: 14),
            Expanded(child: Text(value, style: TextStyle(fontSize: 14, color: isDark ? Colors.white : Colors.black87))),
          ],
        ),
      ),
    );
