...
class _PhoneLoginScreenState extends State<PhoneLoginScreen> {
  ...
  /// Ek hi jagah se saare pin box ka look — kam code, kam galti ki gunjaish
  PinTheme _pin(Color text, Color fill, Color border, {double width = 1}) {
    return PinTheme(
      width: 46,
      height: 54,
      textStyle:
          TextStyle(fontSize: 21, fontWeight: FontWeight.w800, color: text),
          ...
          child: Column(
            ...
            children: [
              ...
              if (!_sent) ...[
                ...
                // 6 alag boxes — jaisa Paytm/PhonePe me hota hai.
                // Sirf wahi parameters use kiye hain jo pinput ke har version
                // me hain, taaki build kabhi na toote.
                Center(
                  child: Pinput(
                    controller: _otp,
                    length: 6,
                    defaultPinTheme: _pin(text, card, line),
                    focusedPinTheme: _pin(text, card, _gold, width: 2),
                    submittedPinTheme:
                        _pin(text, _gold.withOpacity(0.10), _gold),
                    onChanged: (v) {
                      if (_error.isNotEmpty && v.isNotEmpty) {
                        setState(() => _error = '');
                      }
                    },
                    // 6 digit poore hote hi khud verify — button dabana na pade
