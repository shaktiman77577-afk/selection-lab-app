...
class DescriptiveResultScreen extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 14),
          _annotatedAnswer(t, r['your_answer']?.toString() ?? '', score),
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text('Grammar & spelling notes',
                    style: TextStyle(
                        fontSize: 12.5,
                        fontWeight: FontWeight.w800,
                        color: t.text)),
              ),
              if (_d(score['grammar_deduction']) > 0)
                Text('Total −${_fmt(_d(score['grammar_deduction']))} marks',
                    style: const TextStyle(
                        fontSize: 11.5,
                        fontWeight: FontWeight.w800,
  ...
  Widget _annotatedAnswer(DT t, String body, Map<String, dynamic> score) {
    const blue = Color(0xFF2980B9);
    const red = Color(0xFFC0392B);
    final marks = <Map<String, dynamic>>[];
    void push(dynamic raw, Color c) {
      if (raw is! Map) return;
      final off = int.tryParse('${raw['offset']}') ?? -1;
      if (off < 0 || off >= body.length) return;
      var len = int.tryParse('${raw['length']}') ?? 1;
      if (len < 1) len = 1;
      final end = (off + len) > body.length ? body.length : off + len;
      marks.add({'s': off, 'e': end, 'c': c});
    }

    for (final it in (score['grammar_issues'] as List? ?? [])) {
      push(it, blue);
    }
    for (final it in (score['spelling_issues'] as List? ?? [])) {
      push(it, red);
    }
    marks.sort((a, b) => (a['s'] as int).compareTo(b['s'] as int));
    ...
    for (final m in marks) {
      final st = m['s'] as int;
      final en = m['e'] as int;
      if (st < last) continue; // skip overlaps
      if (st > pos) spans.add(TextSpan(text: body.substring(pos, st)));
      spans.add(TextSpan(
          text: body.substring(st, en),
          style: TextStyle(
            decoration: TextDecoration.underline,
            decorationColor: m['c'] as Color,
            decorationThickness: 2.5,
            backgroundColor: (m['c'] as Color).withOpacity(0.08),
          )));
      pos = en;
      last = en;
    }
    if (pos < body.length) spans.add(TextSpan(text: body.substring(pos)));
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (marks.isNotEmpty) ...[
            const SizedBox(height: 6),
            Text('Blue underline = grammar/punctuation · Red = spelling',
