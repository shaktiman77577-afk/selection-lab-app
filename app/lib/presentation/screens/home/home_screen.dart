...
class _DashboardTabState extends State<DashboardTab> {
  ...
  /// Purana result / solutions ab app ke andar khulte hain.
  ///
  /// Pehle ye Chrome me website kholte the, jahan student ko dobara login
  /// karna padta tha — bahut log wahin chhod dete the.
  void _openReview(int testId) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => MockReviewScreen(testId: testId, userId: uid),
      ),
    );
  }
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    [
                      'Tier 2',
                      () => Navigator.push(context,
                          MaterialPageRoute(builder: (_) => const Tier2Screen()))
                    ],
                    [
                      'Blog',
                      () => Navigator.push(context,
  ...
  Widget _courseSlide(Map<String, dynamic> c, String img) =>
      _posterSlide(img, () => _openCourse(c));

  Widget _bannerSlide(String img, String link) {
    return _posterSlide(img, link.isEmpty ? null : () => _open(link));
  }

  /// Poster ko POORA dikhata hai — website jaisa hi.
  ///
  /// Pehle BoxFit.cover tha, isse poster ke kinare (price, "LIMITED OFFER"
  /// jaisi cheezein) kat jate the. Ab asli image `contain` se aati hai aur
  /// peeche usi image ki blurred copy bharti hai, taaki khali patti na dikhe.
  Widget _posterSlide(String img, VoidCallback? onTap) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 2),
      child: GestureDetector(
        onTap: onTap,
                    ...
                    child: img.isEmpty
                        ? Container(
                            ...
                            child: const Icon(Icons.school,
                                color: kDGold, size: 40))
                        // Card me bhi poora poster — kinare (price, offer)
                        // kat na jayein
                        : SizedBox(
                            height: imgH,
                            width: double.infinity,
