...
/// Reusable Razorpay payment handler for courses, PDFs, mock series,
...
class RazorpayService {
  ...
  Future<void> payForDescriptive(
    ...
    try {
      ...
      _openCheckout(
        ...
        amount: data['amount'],
        name: data['title'] ?? 'Selection Lab',
        description: 'Descriptive Test Series',
        userName: userName,
        userEmail: userEmail,
      );
    } catch (e) {
  ...
  // MOCK TEST SERIES PAYMENT (uses /payments/series-order + /payments/verify-series)
  Future<void> payForMockSeries(
    BuildContext context, {
    required int userId,
    required int seriesId,
    required String userName,
  ...
  // ── TIER 2 PRACTICE PAYMENT (uses /tier2/order + /tier2/verify) ─────────────
  // Descriptive/mock jaisa hi flow. Backend ka product_type "tier2" hai, isliye
  // alag order aur verify endpoint hain — payments/series-order isko nahi
  // sambhalta.
  Future<void> payForTier2(
    BuildContext context, {
    required int userId,
    required int seriesId,
    required String userName,
    ...
    try {
      final res = await http.post(
        Uri.parse('${AppConstants.apiUrl}/tier2/order'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'user_id': userId, 'series_id': seriesId, 'coupon_code': couponCode}),
      ).timeout(const Duration(seconds: 15));
  ...
  Future<void> payForLive(
    BuildContext context, {
    required int userId,
    required String kind, // 'batch' | 'class'
    required int itemId,
    required String userName,
    required String userEmail,
    required void Function() onSuccess,
    ...
    try {
      final res = await http.post(
        Uri.parse('${AppConstants.apiUrl}/live/order'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'user_id': userId, 'kind': kind, 'item_id': itemId}),
      ).timeout(const Duration(seconds: 15));
      ...
      _openCheckout(
        ...
        amount: data['amount'],
        name: data['title'] ?? 'Selection Lab',
        description: kind == 'batch' ? 'Live Class Batch' : 'Live Class',
        userName: userName,
        userEmail: userEmail,
      );
    } catch (e) {
